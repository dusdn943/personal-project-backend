...
public class JwtAuthenticationFilter extends OncePerRequestFilter {
    ...
    protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain filterChain)
        ...
        // 1. AccessToken 가져온거 꺼내고,
        // 		Request Header 에 Authorization 이라는 이름으로 담아서 보낸다.
        // 		request.getHeader 했을 때 있으면 나오고, 없으면 null 이 나온다.
        //      Bearer ~~~~~~~~~~~~~~~~~~
        try {
            ...
            // 2. 그리고 그걸 검증(사인 확인, 유효기간 체크하기)
            if (jwtHelper.validation(accessToken)) {
                filterChain.doFilter(request, response); // 통과시킨다.
            } else {
                // 검증 실패했으면 에러(401 UNAUTHORIZED)
                response.setStatus(HttpServletResponse.SC_UNAUTHORIZED);
            }
        } catch (RuntimeException ex) {
            // 토큰을 꺼내거나, 검증하는 과정에서 에러가 나면 로그를 찍고 돌려 보낸다.
